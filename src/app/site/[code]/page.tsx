...
export default function SitePage() {
  ...
  const code = params?.code ?? ""; // guard if undefined

  // ALWAYS default to arrays/null in state
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(`/api/proxy/meters?site_code=${encodeURIComponent(code)}`, { cache: "no-store" });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(`/api/proxy/series?meter_id=${encodeURIComponent(selectedMeter.meter_id)}&hours=24`, { cache: "no-store" });
        const j = await r.json().catch(() => []);
    ...
    (async () => {
      try {
        const r = await fetch(`/api/proxy/tariffs?meter_id=${encodeURIComponent(selectedMeter.meter_id)}`, { cache: "no-store" });
  ...
  // Compute “cost” overlay per point from interval series + tariff (ALWAYS return an array)
  ...
  // Billing fetch (site-level) with guards
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(`/api/proxy/billing/summary?site_code=${encodeURIComponent(code)}&from=${from}&to=${to}`, { cache: "no-store" });
        const j = await r.json().catch(() => null);
        setBill(asObject(j));
      } catch (e) {
              ...
              {selectedMeter && (
                <form
                  ...
                  onSubmit={async (e:any) => {
                    ...
                    try {
                      const r = await fetch('/api/proxy/tariffs', {
